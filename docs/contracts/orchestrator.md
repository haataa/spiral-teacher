...
Phase 2: 逐概念对话
  ├── 按 min_importance 过滤 teaching_order（跳过低重要性概念）
  for concept in filtered_teaching_order:
...
对话开始前，按 `config.min_importance`（默认 3）过滤 `teaching_order`，跳过 importance 低于阈值的概念。这些概念不参与对话，不消耗轮次。

### 终止条件

1. **正常完成**：所有过滤后概念的 Learner feedback.type == understood
...
8. min_importance 过滤：低于阈值的概念被跳过，不出现在对话中
